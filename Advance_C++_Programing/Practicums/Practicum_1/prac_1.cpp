/*
Опр: Инкапсуляция - скрытие состояния объекта
Опр: Наследование – концепция ООП, которая позволяет определять новые классы как расширения существующих
Опр: Различают открытое (А является B) и закрытое (А содержит B)
Опр: Открытое наследование = контракт, где мы обещаем клиенту, что класс поддерживает открытые методы класса 
     родителя, сохраняет инварианты и тп
Опр: Полиморфизм – концепция ООП, которая позволяет для единого интерфейса представить множество реализаций 
    в зависимости от аргументов.
Опр: Статический полиморфизм – перегрузки функций, шаблоны
Опр: Динамический полиморфизм – виртуальные функции     
*/
