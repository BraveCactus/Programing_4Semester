...
template <typename T>
size_t regisrty<T>::count(0);

template <typename T>
T* regisrty<T>::head(nullptr);


class BusForChildren :
        public Automobile<BusForChildren>,
        regisrty<BusForChildren> {
    friend regisrty<BusForChildren>;

public:
    void drive_impl() {
        std::cout << "carefully driving  bus" << std::endl;
    }
};
